/* Wanderer’s Atlas — Final Mark with Leaflet Skyrim Location Layer */
(() => {
  "use strict";
  ...
  const SKYRIM_LOCATION_PACK_URL = "data/skyrim-location-pack.json";
  ...
  const state = {
    ...
    skyrimLocationLayerVisible: false,
    skyrimLocationPackLoaded: false,
    skyrimLocationFeatures: []
  };
  ...
  const skyrimLocationLayer = L.layerGroup();
  ...
  const el = {
    ...
    skyrimLocationLayerToggle: document.getElementById("skyrimLocationLayerToggle")
  };
  ...
  function init() {
    ...
    updatePanelState();
    setupBuildBadge();
    applySkyrimLocationLayerSetting();
  ...
  function bindEvents() {
    ...
    el.skyrimLocationLayerToggle.addEventListener("change", toggleSkyrimLocationLayer);
  ...
  function applySkyrimLocationLayerSetting() {
    el.skyrimLocationLayerToggle.checked = !!state.skyrimLocationLayerVisible;

    if (state.skyrimLocationLayerVisible) {
      ensureSkyrimLocationLayer()
        .then(() => skyrimLocationLayer.addTo(map))
        .catch(error => {
          console.error("Skyrim location layer failed", error);
          state.skyrimLocationLayerVisible = false;
          el.skyrimLocationLayerToggle.checked = false;
          saveSettings();
          setStatus("Could not load Skyrim locations.");
        });
    }
  }

  async function toggleSkyrimLocationLayer() {
    const visible = Boolean(el.skyrimLocationLayerToggle.checked);
    state.skyrimLocationLayerVisible = visible;
    saveSettings();

    if (!visible) {
      skyrimLocationLayer.removeFrom(map);
      setStatus("Skyrim locations hidden.");
      return;
    }

    try {
      await ensureSkyrimLocationLayer();
      skyrimLocationLayer.addTo(map);
      setStatus(`Skyrim locations shown: ${state.skyrimLocationFeatures.length}.`);
    } catch (error) {
      console.error("Skyrim location layer failed", error);
      el.skyrimLocationLayerToggle.checked = false;
      state.skyrimLocationLayerVisible = false;
      saveSettings();
      alert(error.message || "Could not load Skyrim locations.");
      setStatus("Could not load Skyrim locations.");
    }
  }

  async function ensureSkyrimLocationLayer() {
    if (state.skyrimLocationPackLoaded) return;

    const response = await fetch(SKYRIM_LOCATION_PACK_URL, { cache: "no-store" });

    if (!response.ok) {
      throw new Error("Could not load the Skyrim location layer.");
    }

    const pack = await response.json();

    if (Number(pack.map?.width) && Number(pack.map.width) !== MAP_WIDTH) {
      throw new Error("The Skyrim location layer width does not match this atlas map.");
    }

    if (Number(pack.map?.height) && Number(pack.map.height) !== MAP_HEIGHT) {
      throw new Error("The Skyrim location layer height does not match this atlas map.");
    }

    state.skyrimLocationFeatures = Array.isArray(pack.features) ? pack.features : [];
    renderSkyrimLocationLayer();
    state.skyrimLocationPackLoaded = true;
  }

  function renderSkyrimLocationLayer() {
    skyrimLocationLayer.clearLayers();

    state.skyrimLocationFeatures.forEach(feature => {
      const point = feature.points?.[0];
      ...
      const marker = L.marker([y, x], {
        ...
        icon: L.divIcon({
          className: "",
          html: `<div class="skyrim-location-marker"><img src="sigils/15.png" alt="" aria-hidden="true"></div>`,
          iconSize: [26, 26],
          iconAnchor: [13, 13]
        })
      });

      marker.bindTooltip(escapeHtml(feature.title || "Skyrim Location"), {
        direction: "top",
        offset: [0, -12],
        className: "skyrim-location-tooltip"
      });

      skyrimLocationLayer.addLayer(marker);
    });
  }
  ...
  function loadState() {
    try {
      ...
      state.skyrimLocationLayerVisible = !!settings.skyrimLocationLayerVisible;
  ...
  function saveSettings() {
    localStorage.setItem(SETTINGS_KEY, JSON.stringify({
      ...
      skyrimLocationLayerVisible: state.skyrimLocationLayerVisible
    }));
  }
