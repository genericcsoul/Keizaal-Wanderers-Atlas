/* Mark 5 Cleanup 2A — Supabase Configured. 7-character online codes enabled; stable Mark 5 behavior retained. */
(() => {
  "use strict";
  ...
  function bindEvents() {
    ...
    el.copyShareBtn.addEventListener("click", copyShareCode);
  ...
  function renderDomFeature(feature, svg) {
    ...
    if (feature.type === "marker") {
      ...
      marker.type = "button";
      marker.className = `dom-entry dom-marker${selected ? " is-selected" : ""}`;
      ...
      marker.innerHTML = tooltipHtml(feature);
  ...
  function renderDomDraft(svg) {
    ...
    if (state.draftFeature?.type === "marker") {
      ...
      marker.className = "dom-entry dom-marker is-selected";
      marker.style.left = `${screen.x}px`;
      marker.style.top = `${screen.y}px`;
      marker.style.backgroundColor = category.color;
      domLayer.appendChild(marker);
  ...
  function updateShortShareStatus() {
    ...
    if (isShortCodeConfigured()) {
      el.shortShareStatus.textContent = "7-character online codes are enabled. Offline codes remain available as backup.";
      el.copyShortShareBtn.disabled = false;
    } else {
      el.shortShareStatus.textContent = "7-character codes require Supabase setup in atlas-config.js. Offline codes always work.";
      el.copyShortShareBtn.disabled = false;
    }
  }
  ...
  async function saveShortSharePayload(payload) {
    if (!isShortCodeConfigured()) {
      throw new Error("7-character codes are not configured yet. Fill in atlas-config.js and run supabase-schema.sql in Supabase.");
    }
  ...
  async function fetchShortSharePayload(code) {
    if (!isShortCodeConfigured()) {
      throw new Error("7-character codes are not configured yet. Use an offline code, or fill in atlas-config.js and run supabase-schema.sql.");
    }
    ...
    if (!Array.isArray(rows) || !rows.length) {
      throw new Error("No shared entries were found for that 7-character code.");
    }

    const row = rows[0];

    if (row.expires_at && new Date(row.expires_at).getTime() < Date.now()) {
      throw new Error("That 7-character code has expired.");
    }
  ...
  async function copyShortShareCode() {
    ...
    if (!isShortCodeConfigured()) {
      alert("7-character codes need Supabase setup first. Fill in atlas-config.js, run supabase-schema.sql, then upload again. Use Copy Offline Code for now.");
      return;
    }
    ...
    try {
      ...
      setStatus(`7-character code copied: ${code}`);
    } catch (error) {
      console.error("Short code creation failed", error);
      alert(error.message || "Could not create a 7-character code.");
      setStatus("Could not create 7-character code.");
    } finally {
      el.copyShortShareBtn.disabled = false;
  ...
  // Share code serialization.
  function makeShareCode(features = getShareableFeatures()) {
    const payload = compactPayload(features);
    return "WA2." + encodePayload(payload);
  }

  function compactPayload(features) {
    return {
      v: 2,
      app: "Wanderer’s Atlas",
      w: MAP_WIDTH,
  ...
  function decodeCompactFeature(item) {
    if (!Array.isArray(item)) return null;

    return {
      id: item[0],
      type: decodeFeatureType(item[1]),
      category: item[2],
      title: item[3],
      confidence: item[4],
      creator: item[5],
      notes: item[6],
      points: Array.isArray(item[7]) ? item[7].map(point => ({ x: point[0], y: point[1] })) : [],
      source: item[8] || "personal",
      createdAt: item[9],
      updatedAt: item[10]
    };
  }

  function encodeFeatureType(type) {
    return type === "route" ? "t" : type === "range" ? "r" : "m";
  }

  function decodeFeatureType(type) {
    return type === "t" ? "route" : type === "r" ? "range" : "marker";
  }
  ...
  async function copyShareCode() {
    ...
    const code = makeShareCode(features);

    if (code.length > SECURITY_LIMITS.maxShareCodeChars) {
      alert(`This share code is too large. Select fewer entries or shorten long notes. Limit: ${formatBytes(SECURITY_LIMITS.maxShareCodeChars)}.`);
      return;
    }

    await copyTextToClipboard(code);
    setStatus("Offline share code copied.");
  }

  // Receive dialog controls.
  ...
  async function parseReceiveInput(raw) {
    ...
    if (isShortCodeInput(trimmed)) {
      payload = await fetchShortSharePayload(trimmed);
    } else if (trimmed.startsWith("WA2.")) {
      payload = decodePayload(trimmed.slice(4));
    } else if (trimmed.startsWith("WA1.")) {
      payload = decodePayload(trimmed.slice(4));
    } else if (trimmed.startsWith("{") || trimmed.startsWith("[")) {
      payload = JSON.parse(trimmed);
    } else {
      payload = decodePayload(trimmed);
    }
  ...
  function normalizeReceivePayload(payload) {
    ...
    const rawFeatures = Array.isArray(payload.f)
      ? payload.f.map(decodeCompactFeature)
